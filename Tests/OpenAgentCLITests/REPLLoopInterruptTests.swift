...
// MARK: - Mock Output Stream for Capturing Interrupt Output

/// Captures all output written during REPLLoop tests for assertion.
///
/// Thread-safe output capture using NSLock, matching the pattern used
/// in other test files (MockPermissionOutput).
...
// MARK: - Signal-Injected Mock Input Reader

/// A mock input reader that can simulate signal delivery between reads.
///
/// Provides predefined input lines and supports injecting a signal event
/// that the REPLLoop should detect. This allows testing interrupt behavior
/// without sending real OS signals during test execution.
///
/// Usage:
///   let reader = SignalMockInputReader(
///       lines: ["query", "/exit"],
///       signalAfterRead: 1,   // Trigger signal after first read
///       signal: .interrupt     // Type of signal to inject
///   )
final class SignalMockInputReader: InputReading, @unchecked Sendable {
    ...
    /// The signal to inject; nil means no signal injection.
    let signalToInject: SignalEvent?

    /// The read index after which to set the signal flag (0-based).
    /// The signal becomes visible to SignalHandler.check() after this many reads.
    let signalAfterRead: Int

    /// Whether the signal has been injected.
    ...
    func readLine(prompt: String) -> String? {
        ...
        // Inject signal after the specified read completes.
        // signalAfterRead is 1-indexed: signalAfterRead=1 means "after the first read".
        // We inject AFTER incrementing callCount so the signal is visible when
        // the REPL processes the returned line (e.g. during stream iteration).
        if callCount == signalAfterRead && !signalInjected {
            signalInjected = true
            // Inject signal via the test-only API
